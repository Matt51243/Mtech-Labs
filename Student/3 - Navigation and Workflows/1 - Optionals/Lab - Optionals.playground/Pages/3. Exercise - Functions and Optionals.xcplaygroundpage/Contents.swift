...
func checkAge(age: String) -> Int? {
    if let age = Int(age) {
        if age >= 18 {
            ...
            print("Sorry your not old enough to use our app")
        }
    } else {
        print("can you please re-enter your age")
    }
...
let itemName: String? = stock.first?.key

func cost(itemName: String) -> Double? {
    if let itemCount = stock[itemName], itemCount > 0 {
        return prices[itemName]
    }
    return nil
}
print(cost(itemName: "Chips"))
print(cost(itemName: "String Cheese"))
/*:
